...
class AbyssalConversionCosmos : BaseSpecialItemPlugin() {
    ...
    override fun init(stack: CargoStackAPI) {
        ...
        hullmodSpec = Global.getSettings().getHullModSpec("rat_cosmos_conversion")
    }
...
//		y = cy - h / 2f;
    ...
    override fun createTooltip(tooltip: TooltipMakerAPI, expanded: Boolean, transferHandler: CargoTransferHandlerAPI?, stackSource: Any?) {
        ...
        tooltip.addPara("Converts an automated abyssal hull into one that is merely AI assisted, with specialised bridge for a small crew of humans. This is not applicable if the ship has a permanently integrated AI core.", 0f, Misc.getTextColor(), Misc.getHighlightColor(),
            "automated abyssal hull", "bridge")
        tooltip.addSpacer(5f)

        tooltip.addPara("This specific conversion integrates a cosmos core into the hull, allowing the usage of the shipsystem related to that core.", 0f, Misc.getTextColor(), Misc.getHighlightColor(),
            "cosmos core")
        tooltip.addSpacer(5f)
    ...
    override fun performRightClickAction() {
        ...
        var listener = object : FleetMemberPickerListener {
            override fun pickedFleetMembers(members: MutableList<FleetMemberAPI>?) {
                ...
                {
                    Global.getSector().playerFleet.cargo.addSpecial(SpecialItemData("rat_cosmos_integration", null), 1f)
                }
            }
